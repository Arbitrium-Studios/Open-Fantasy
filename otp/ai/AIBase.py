...
class AIBase:
    ...
    def setupCpuAffinities(self, minChannel):
        if game.name == 'uberDog':
            affinityMask = ConfigVariableInt('uberdog-cpu-affinity-mask', -1)
        else:
            affinityMask = ConfigVariableInt('ai-cpu-affinity-mask', -1)
        if affinityMask != -1:
            ...
            if game.name == 'uberDog':
                ...
                affinity = ConfigVariableInt('ai-cpu-affinity', -1)
                if autoAffinity and affinity == -1:
                    affinity = 1
            if affinity != -1:
