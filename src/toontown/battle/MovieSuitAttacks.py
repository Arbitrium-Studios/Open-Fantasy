...
def doPoundKey(attack):
    ...
    propTrack += [Wait(dialDuration), Func(receiver.wrtReparentTo, phone), Wait(finalPhoneDelay), LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_NEARZERO), Func(MovieUtil.removeProps, [receiver, phone])]
...
def doHeadShrink(attack):
    ...
    damageAnims.append(['conked',
     ...
     0.4])
    toonTrack = getToonTrack(attack, damageDelay=damageDelay, splicedDamageAnims=damageAnims, dodgeDelay=dodgeDelay, dodgeAnimNames=['sidestep'])
    multiTrackList = Parallel(suitTrack, sprayTrack, cloudTrack, dropTrack, toonTrack)
    if dmg > 0:
        ...
        multiTrackList.append(shrinkTrack)
...
def doBounceCheck(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay[suitType], ['conked'], dodgeDelay[suitType], ['sidestep'])
    soundTracks = Sequence(getSoundTrack('SA_pink_slip.ogg', delay=throwDelay[suitType] + 0.5, duration=0.6, node=suit), getSoundTrack('SA_pink_slip.ogg', delay=0.4, duration=0.6, node=suit))
