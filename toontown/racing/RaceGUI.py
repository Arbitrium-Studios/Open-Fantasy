...
class RaceGUI:
    ...
    class RacerInfo:
        ...
        def update(self, curvetime=None, maxlaphit=None, faceX=None,
                   mapspotPt=None, place=None, finished=None):
            if self.enabled:
    ...
    def initRaceMode(self):
        ...
        self.lapLabel = DirectLabel(
            relief=None, pos=(
                1.1, 0, 0.45), text='1/' + str(
                self.race.lapCount), text_scale=0.1, text_fg=(
                0.95, 0.95, 0, 1), text_font=ToontownGlobals.getSignFont())
        ...
        self.wrongWayLabel = DirectLabel(
            relief=None, pos=(
                1.1, 0, 0.85), text=TTLocalizer.KartRace_WrongWay, text_scale=0.1, text_fg=(
                0.95, 0, 0, 1), text_font=ToontownGlobals.getSignFont())
        ...
        self.gagPanel = DirectFrame(parent=self.raceModeRoot, relief=None, image=loader.loadModel(
            'phase_6/models/karting/gag_panel'), image_scale=0.25, pos=(-1.13, 0, -0.5))
    ...
    def update(self, time):
        ...
        if not localRacer.finished and self.faceEndPos[0] - \
                localRacer.face.getX() < nearDiff:
    ...
    def racerFinished(self, avId, trackId, place, totalTime, entryFee,
                      qualify, winnings, bonus, trophies, circuitPoints, circuitTime):
        racer = self.racerDict.get(avId, None)
        if racer:
    ...
    def racerFinishedCircuit(self, avId, place, entryFee,
                             winnings, bonus, trophies):
        racer = self.racerDict.get(avId, None)
        if racer:
