...
class RaceResultsPanel(DirectFrame):
    ...
    def __init__(self, numRacers, race, raceEndPanel, *args, **kwargs):
        ...
        self.pointsLabel = DirectLabel(
            parent=self, relief=None, pos=(
                0.7, 0, 0.3), text=TTLocalizer.KartRace_CircuitPoints, text_fg=(
                0.0, 0.0, 0.0, 1.0), text_scale=TTLocalizer.REPsmallLabel)
    ...
    def displayRacer(self, place, headFrame, name, time, qualify,
                     tickets, bonus, trophies, circuitPoints, circuitTime):
        ...
        def flipText(flip, label=self.entryList[place - 1][3],
                     timeStr=timeStr, recStr=TTLocalizer.KartRace_Record):
        ...
        if not circuitPoints == []:
            ...
            def totalPointTicker(
                    t, label=self.entryList[place - 1][5], current=currentPoints, new=newPoints):
                label['text'] = '%s' % int(currentPoints + t * new)

            def newPointTicker(
                    t, label=self.entryList[place - 1][6], new=newPoints):
    ...
    def updateWinnings(self, place, newTotalTickets):
        ...
        if winnings != 0:
            ...
            def ticketTicker(
                    t, label=self.entryList[place - 1][4], tickets=newTickets):
                label['text'] = TTLocalizer.KartRace_TicketPhrase % int(
                    t * tickets)
...
class RaceEndPanel(DirectFrame):
    ...
    def displayRacer(self, place, entryFee, qualify, winnings, track,
                     bonus, trophies, headFrame, name, time, circuitPoints, circuitTime):
    ...
    def updateWinningsFromCircuit(
            self, place, entryFee, winnings, bonus, trophies=()):
    ...
    def startWinningsPanel(self, entryFee, winnings, track,
                           bonus=None, trophies=(), endOfCircuitRace=False):
        if not self.enabled:
            return
        taskMgr.remove('showExitButton')
        try:
