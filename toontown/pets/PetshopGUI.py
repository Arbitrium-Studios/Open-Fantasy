    ...
    class NamePicker(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeed, gender):
            ...
            self.alphabetList = self.makeScrollList(
                self.gui, (-0.012, 0, -0.075), (1, 0.8, 0.8, 1), self.letters, self.makeLabel, [
        ...
        def rebuildNameList(self):
            ...
            self.nameList = self.makeScrollList(
                self.gui, (0.277, 0, -0.075), (1, 0.8, 0.8, 1), self.curNames, self.makeLabel, [
        ...
        def makeScrollList(self, gui, ipos, mcolor, nitems,
                           nitemMakeFunction, nitemMakeExtraArgs, nVisibleItems):
            ...
            def showDialog(avatar):
                ...
                self.cancelButton = DirectButton(
                    ...
                    command=lambda: messenger.send(
                        ...
                        [0]))
                self.okButton = DirectButton(
                    parent=self, relief=None, image=okImageList, geom=checkIcon, scale=modelScale, text=(
                        '', TTLocalizer.PetshopReturn), text_pos=(
                        5.8, 4.4), text_scale=0.7, pressEffect=False, command=lambda: messenger.send(
                        doneEvent, [1]))
                self.petView = self.attachNewNode('petView')
    ...
    class ChoosePetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeeds):
            ...
            self.cancelButton = DirectButton(parent=self,
                                             ...
                                             command=lambda: messenger.send(doneEvent,
                                                                            [-1]))
            self.pawLButton = DirectButton(parent=self,
                                           relief=None,
                                           image=pawLImageList,
