...
class DistributedPetAI(DistributedSmoothNodeAI.DistributedSmoothNodeAI,
    ...
    def __init__(self, air, dna=None):
        ...
        self.traitSeed = 0
        self.safeZone = ToontownGlobals.ToontropolisCenter
        self.initialDNA = dna
        self.active = 1
        self.activated = 0
        ...
        self.gaitFSM = ClassicFSM.ClassicFSM('petGaitFSM', [State.State('off', self.gaitEnterOff, self.gaitExitOff),
                                                            State.State(
            'neutral', self.gaitEnterNeutral, self.gaitExitNeutral),
    ...
    def _initDBVals(self, ownerId, name=None, traitSeed=0,
                    dna=None, safeZone=ToontownGlobals.ToontropolisCenter):
    ...
    def announceZoneChange(self, newZoneId, oldZoneId):
        ...
        if self.estateOwnerId:
            ...
            self.inEstate = 0
            self.estateZones = []
        PetObserve.send(list(broadcastZones.keys()), PetObserve.PetActionObserve(
    ...
    def handleAvPetInteraction(self, mode, avId):
        if mode not in (PetConstants.PET_MOVIE_SCRATCH,
                        PetConstants.PET_MOVIE_FEED, PetConstants.PET_MOVIE_CALL):
