...
class PartyEditorGrid:
    ...
    def checkGridSquareForAvailability(self, gridSquare, size):
        ...
        for y in range(
            ...
            for x in range(int(gridSquare.x - size[0] / 2) + xOffsetLow, int(
                    gridSquare.x + size[0] / 2) + xOffsetHigh):
    ...
    def registerNewElement(self, gridElement, centerGridSquare, size):
        ...
        for y in range(int(centerGridSquare.y -
                       size[1] / 2), int(centerGridSquare.y + size[1] / 2) + yOffset):
            for x in range(int(centerGridSquare.x - size[0] / 2) + xOffsetLow, int(
                    centerGridSquare.x + size[0] / 2) + xOffsetHigh):
                testGridSquare = self.getGridSquare(x, y)
                if testGridSquare is None:
                    return False
                if testGridSquare.gridElement is not None:
