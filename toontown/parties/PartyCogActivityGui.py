...
class PartyCogTrackerGui:
    ...
    def __init__(self):
        self.cogTracker = loader.loadModel(
            '../../user/default/resources/default/phase_13/models/parties/cogTrackerGUI')
        ...
        for i in range(3):
            ...
            seq = Sequence(
                LerpScaleInterval(
                    self.cogs[i], duration=0.1, scale=big, startScale=Point3(
                        1.0, 1.0, 1.0), blendType='easeOut'), LerpScaleInterval(
                    self.cogs[i], duration=0.25, scale=Point3(
                        1.0, 1.0, 1.0), startScale=big, blendType='easeOut'), Wait(0.4))
...
class PartyCogActivityGui(DirectObject):
    ...
    def _initVictoryBalanceBar(self):
        ...
        victoryBalanceBar = loader.loadModel(
            '../../user/default/resources/default/phase_13/models/parties/tt_m_gui_pty_pieToss_balanceBar')
    ...
    def _initControlGui(self):
        ...
        pieTossControls = loader.loadModel(
            '../../user/default/resources/default/phase_13/models/parties/tt_m_gui_pty_pieToss_controls')
    ...
    def _initScore(self):
        ...
        self._scoreTitle = OnscreenText(
            text=TTLocalizer.PartyCogGuiScoreLabel, pos=PartyGlobals.CogActivityScoreTitle, scale=0.05, fg=(
                1.0, 1.0, 1.0, 1.0), align=TextNode.ARight)
    ...
    def _initSpamWarning(self):
        self._spamWarning = OnscreenText(
            text=TTLocalizer.PartyCogGuiSpamWarning, scale=0.15, fg=(
    ...
    def showAttackControls(self):
        if self._attackKeys.isHidden():
            ...
            Sequence(
                ToontownIntervals.getPulseLargerIval(
                    self._attackKeys, '', scale=0.15), Wait(
    ...
    def showMoveControls(self):
        if self._moveKeys.isHidden() and not self._attackKeys.isHidden():
            ...
            Sequence(
                ToontownIntervals.getPulseLargerIval(
                    self._moveKeys, '', scale=0.15), Wait(
                    PartyGlobals.CogActivityControlsShowTime), Func(
