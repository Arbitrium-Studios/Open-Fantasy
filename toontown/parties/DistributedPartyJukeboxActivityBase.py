...
class DistributedPartyJukeboxActivityBase(DistributedPartyActivity):
    ...
    def load(self):
        ...
        self.jukebox = Actor('../../user/default/resources/default/phase_13/models/parties/jukebox_model',
                             {'dance': '../../user/default/resources/default/phase_13/models/parties/jukebox_dance'})
    ...
    def queuedSongsResponse(self, songInfoList, index):
        if self.gui.isLoaded():
            for i in range(len(songInfoList)):
                ...
                self.__addSongToQueue(
                    songInfo, isLocalQueue=index >= 0 and i == index)
