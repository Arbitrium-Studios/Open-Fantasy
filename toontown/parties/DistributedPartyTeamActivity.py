...
class DistributedPartyTeamActivity(DistributedPartyActivity):
    ...
    def __init__(self, cr, activityId,
                 startDelay=PartyGlobals.TeamActivityStartDelay, balanceTeams=False):
    ...
    def d_toonJoinRequest(self, team):
        ...
        if self.activityFSM.state in [
                'WaitForEnough', 'WaitToStart'] and self._localToonRequestStatus is None:
    ...
    def exitRequestDenied(self, reason):
        ...
        if reason == PartyGlobals.DenialReasons.Default:
            self.showMessage(
                ...
                self.getTitle())
        if self.isLocalToonPlaying and (self.isState(
                'WaitToStart') or self.isState('WaitForEnough')):
            self.teamActivityGui.enableExitButton()
    ...
    def switchTeamRequestDenied(self, reason):
        ...
        if reason == PartyGlobals.DenialReasons.Full:
            ...
            self.showMessage(
                ...
                endState='activity')
        if self.isLocalToonPlaying and (self.isState('WaitToStart') or self.isState(
    ...
    def isState(self, state):
        return hasattr(
            self, 'activityFSM') and self.activityFSM.getCurrentOrNextState() == state
