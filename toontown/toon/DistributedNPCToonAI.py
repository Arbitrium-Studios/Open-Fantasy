...
class DistributedNPCToonAI(DistributedNPCToonBaseAI):
    FourthGagVelvetRopeBan = config.GetBool(
        'want-ban-fourth-gag-velvet-rope', 0)
    ...
    def chooseQuest(self, questId):
        avId = self.air.getAvatarIdFromSender()
        self.notify.debug(
            'chooseQuest: avatar %s choseQuest %s' %
            (avId, questId))
        ...
        if self.pendingQuests is None:
            ...
            self.air.writeServerEvent(
                'suspicious', avId, 'unexpected chooseQuest')
            return
        if questId == 0:
            self.pendingAvId = None
    ...
    def chooseTrack(self, trackId):
        avId = self.air.getAvatarIdFromSender()
        self.notify.debug(
            'chooseTrack: avatar %s choseTrack %s' %
            (avId, trackId))
        if not self.pendingAvId:
            self.notify.warning(
        ...
        if self.pendingTracks is None:
            self.notify.warning(
                ...
                avId)
            self.air.writeServerEvent(
                'suspicious', avId, 'unexpected chooseTrack')
            return
        if trackId == -1:
            self.pendingAvId = None
    ...
    def setMovieDone(self):
        ...
        if avId in self.busy:
            ...
            self.air.writeServerEvent(
                'suspicious',
                avId,
                'DistributedNPCToonAI.setMovieDone busy with %s' %
                self.busy)
            self.notify.warning(
