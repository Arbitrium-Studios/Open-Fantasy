...
class ToonAvatarPanel(AvatarPanelBase.AvatarPanelBase):
    ...
    def __init__(self, avatar, playerId=None):
        ...
        self.frame = DirectFrame(
            ...
            relief=None,
            pos=(-0.22, 100, -0.47), parent=base.a2dTopRight)
    ...
    def __petDetailsLoaded(self, avatar):
        ...
        if avatar:
            self.notify.debug(
                "Looking at someone's pet. pet doId = %s" %
                avatar.doId)
    ...
    def handleKick(self):
        if not base.cr.playGame.getPlace().getState() == 'elevator':
            self.confirmKickOutDialog = TTDialog.TTDialog(
                style=TTDialog.YesNo, text=TTLocalizer.BoardingKickOutConfirm %
                self.avName, command=self.__confirmKickOutCallback)
    ...
    def __checkGroupStatus(self):
        ...
        if hasattr(self, 'avatar'):
            if self.avatar and hasattr(self.avatar, 'getZoneId') and localAvatar.getZoneId(
