...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon,
                      DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def setInventory(self, inventoryNetString):
        if not self.inventory:
            self.inventory = InventoryNew.InventoryNew(
    ...
    def battleSOS(self, requesterId):
        ...
        if isinstance(avatar, DistributedToon) or isinstance(
            ...
            self.setSystemMessage(
                requesterId, TTLocalizer.MovieSOSWhisperHelp %
                avatar.getName(), whisperType=WhisperPopup.WTBattleSOS)
    ...
    def setDefaultZone(self, zoneId):
        ...
        try:
            ...
            self.defaultZone = ToontownCenter
            return
    ...
    def setTalk(self, fromAV, fromAC, avatarName, chat, mods, flags):
        if self.isLocal() and avatarName == '':
    ...
    def isAvFriend(self, avId):
        return base.cr.isFriend(
            avId) or base.cr.playerFriendsManager.isAvatarOwnerPlayerFriend(avId)
    ...
    def whisperSCToontaskTo(self, taskId, toNpcId,
                            toonProgress, msgIndex, sendToId):
        messenger.send('wakeup')
    ...
    def setWhisperSCToontaskFrom(
            self, fromId, taskId, toNpcId, toonProgress, msgIndex):
    ...
    def getTunnelInToonTrack(self, endX, tunnelOrigin):
        ...
        targetX = self.getX()
        self.setX(self.tunnelCenterOffset + (targetX -
                  self.tunnelCenterOffset) * (1.0 - self.tunnelCenterInfluence))
        self.setHpr(tunnelOrigin, 0, 0, 0)
        pivotNode.setH(-self.pivotAngle)
        return Sequence(Wait(0.8), Parallel(LerpHprInterval(pivotNode, pivotDur, hpr=Point3(0, 0, 0), name=self.uniqueName('tunnelInPivot')), Sequence(Wait(pivotDur - pivotLerpDur), LerpPosInterval(self, pivotLerpDur, pos=Point3(targetX, 0, 0),
                        name=self.uniqueName('tunnelInPivotLerpPos')))), Func(self.wrtReparentTo, render), Func(pivotNode.removeNode), LerpPosInterval(self, straightLerpDur, pos=Point3(endX, endY, 0.1), other=tunnelOrigin, name=self.uniqueName('tunnelInStraightLerp')))
    ...
    def handleTunnelIn(self, startTime, endX, x, y, z, h):
        ...
        self.tunnelTrack = Sequence(
            self.getTunnelInToonTrack(
                endX, tunnelOrigin), Func(
                tunnelOrigin.removeNode), Func(
                self.startSmooth))
    ...
    def getTunnelOutToonTrack(self, startX, startY, tunnelOrigin):
        ...
        def getTargetPos(self=self):
            pos = self.getPos()
            return Point3(self.tunnelCenterOffset + (pos[0] - self.tunnelCenterOffset) * (
                1.0 - self.tunnelCenterInfluence), pos[1], pos[2])

        return Sequence(Parallel(LerpPosInterval(self, straightLerpDur, pos=Point3(startX, pivotY, 0.1), startPos=startPos, other=tunnelOrigin, name=self.uniqueName('tunnelOutStraightLerp')), LerpHprInterval(self, straightLerpDur * 0.8, hpr=Point3(reducedAvH, 0, 0), startHpr=startHpr, other=tunnelOrigin, name=self.uniqueName('tunnelOutStraightLerpHpr'))),
                        Func(self.wrtReparentTo, pivotNode), Parallel(LerpHprInterval(pivotNode, pivotDur, hpr=Point3(-self.pivotAngle, 0, 0), name=self.uniqueName('tunnelOutPivot')), LerpPosInterval(self, pivotLerpDur, pos=getTargetPos, name=self.uniqueName('tunnelOutPivotLerpPos'))), Func(self.wrtReparentTo, render), Func(pivotNode.removeNode))
    ...
    def handleTunnelOut(self, startTime, startX, startY, x, y, z, h):
        ...
        self.tunnelTrack = Sequence(
            Func(
                self.stopSmooth), self.getTunnelOutToonTrack(
                startX, startY, tunnelOrigin), Func(
    ...
    def d_setAnimState(self, animName, animMultiplier=1.0,
                       timestamp=None, extraArgs=[]):
        timestamp = globalClockDelta.getFrameNetworkTime()
        self.sendUpdate('setAnimState', [animName, animMultiplier, timestamp])

    def setAnimState(self, animName, animMultiplier=1.0,
                     timestamp=None, animType=None, callback=None, extraArgs=[]):
    ...
    def reconsiderCheesyEffect(self, lerpTime=0):
        ...
        if hoodId != 0:
            ...
            if hoodId == 1:
                if currentHoodId == ToontownGlobals.ToontownCenter:
                    effect = CENormal
    ...
    def setCatalogSchedule(self, currentWeek, nextTime):
        ...
        if self.isLocal():
            self.notify.debug(
                'setCatalogSchedule(%s, %s)' %
                (currentWeek, nextTime))
            if nextTime:
                serverTime = time.time() + self.cr.getServerDelta()
    ...
    def playSplashEffect(self, x, y, z):
        if localAvatar.zoneId not in [ToontownGlobals.ToontownShipyards, ToontownGlobals.AcornAcres] and (
    ...
    def getTrackProgressAsArray(self, maxLength=15):
        shifts = list(map(operator.rshift, maxLength *
                      [self.trackProgress], list(range(maxLength - 1, -1, -1))))
    ...
    def b_setSpeedChatStyleIndex(self, index):
        realIndexToSend = 0
        if isinstance(index, type(0)) and 0 <= index and index < len(
                speedChatStyles):
    ...
    def setChatMuted(self, chatString, chatFlags,
                     dialogue=None, interrupt=1, quiet=0):
    ...
    def setMail(self, mail):
        DistributedToon.partyNotify.debug(
            'setMail called with %d mail items' %
            len(mail))
        self.mail = []
    ...
    def setInvites(self, invites):
        DistributedToon.partyNotify.debug(
            'setInvites called passing in %d invites.' %
            len(invites))
        self.invites = []
    ...
    def setPartyReplies(self, replies):
        DistributedToon.partyNotify.debug(
            'setPartyReplies called passing in %d parties.' %
            len(replies))
        self.partyReplyInfoBases = []
