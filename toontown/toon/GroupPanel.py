...
class GroupPanel(DirectObject.DirectObject):
    ...
    def __load(self):
        ...
        showImageList = (
            ...
            arrowGui.find('**/tt_t_gui_brd_arrow_down'),
            arrowGui.find('**/tt_t_gui_brd_arrow_hover'))
        self.hideButton = DirectButton(relief=None,
                                       text_pos=(0,
                                                 0.15),
                                       text_scale=0.06,
        ...
        self.showButton = DirectButton(
            relief=None, text=(
                '', TTLocalizer.BoardingGroupShow, TTLocalizer.BoardingGroupShow), text_pos=(
                0.03, 0), text_scale=0.06, text_align=TextNode.ALeft, text_fg=Vec4(
                1, 1, 1, 1), text_shadow=Vec4(
                    0, 0, 0, 1), image=showImageList, image_scale=(
                        0.35, 1, 0.5), pos=(
                            -1.3081, 0, 0.03), scale=1.05, command=self.show)
    ...
    def __getAvatarButton(self, avId):
        ...
        if toon.battleId:
            ...
            if avId == localAvatar.doId:
                self.__forceShow()
        return DirectButton(parent=self.frame, relief=None, image=buttonImage, image_scale=(0.06, 1.0, 0.06), text=toonName, text_align=TextNode.ALeft, text_wordwrap=16, text_scale=0.04, text_pos=(
            0.05, -0.015), text_fg=self.textFgcolor, text1_bg=self.textBgDownColor, text2_bg=self.textBgRolloverColor, text3_fg=self.textBgDisabledColor, pos=(0, 0, 0.2), command=self.__openToonAvatarPanel, extraArgs=[toon, avId])
