...
Phase10AnimList = (('leverReach', 'leverReach'), ('leverPull',
                   'leverPull'), ('leverNeutral', 'leverNeutral'))
Phase12AnimList = ()
...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def getBookActors(self):
        ...
        bookActor = Actor.Actor(
            'phase_3.5/models/props/book-mod', {'book': 'phase_3.5/models/props/book-chan'})
    ...
    def resetHeight(self):
        if hasattr(self, 'style') and self.style:
            ...
            shoulderHeight = ToontownGlobals.legHeightDict[self.style.legs] * \
                bodyScale + \
    ...
    def enterNeutral(self, animMultiplier=1, ts=0,
    ...
    def enterVictory(self, animMultiplier=1, ts=0,
                     callback=None, extraArgs=[]):
        anim = 'victory'
    ...
    def enterJumpSquat(self, animMultiplier=1, ts=0,
                       callback=None, extraArgs=[]):
        if not self.isDisguised:
            if self.playingAnim == 'neutral':
                anim = 'jump-squat'
    ...
    def enterJumpAirborne(self, animMultiplier=1, ts=0,
                          callback=None, extraArgs=[]):
        if not self.isDisguised:
    ...
    def enterJumpLand(self, animMultiplier=1, ts=0,
                      callback=None, extraArgs=[]):
        if not self.isDisguised:
    ...
    def enterSwimHold(self, animMultiplier=1, ts=0,
                      callback=None, extraArgs=[]):
        self.getGeomNode().setPos(0, 0, -2)
        self.nametag3d.setPos(0, -2, 1)
        self.pose('swim', 55)
    ...
    def startBobSwimTask(self):
        ...
        self.swimBobSeq = Sequence(self.getGeomNode().posInterval(1, Point3(0, -
                                                                            3, 3), startPos=Point3(0, -
                                                                                                   3, 4), blendType='easeInOut'), self.getGeomNode().posInterval(1, Point3(0, -
                                                                                                                                                                           3, 4), startPos=Point3(0, -
                                                                                                                                                                                                  3, 3), blendType='easeInOut'))
    ...
    def enterOpenBook(self, animMultiplier=1, ts=0,
                      callback=None, extraArgs=[]):
    ...
    def enterReadBook(self, animMultiplier=1, ts=0,
                      callback=None, extraArgs=[]):
    ...
    def enterCloseBook(self, animMultiplier=1, ts=0,
                       callback=None, extraArgs=[]):
        ...
        self.track = Sequence(
            Func(
                self.showBooks), bookTracks, Func(
                self.hideBooks), name=trackName)
        if callback:
            self.track.setDoneEvent(self.track.getName())
            self.acceptOnce(self.track.getName(), callback, extraArgs)
        self.track.start(ts)
    ...
    def getTeleportOutTrack(self, autoFinishTrack=1):
        ...
        holeTrack = Track(
            (0.0, Func(
                showHoles, holes, hands)), (0.5, SoundInterval(
                    self.getSoundTeleport(), node=self)), (1.708, Func(
                        reparentHoles, holes, self)), (3.4, Func(
                            cleanupHoles, holes)))
        if hasattr(self, 'uniqueName'):
    ...
    def enterTeleportOut(self, animMultiplier=1, ts=0,
                         callback=None, extraArgs=[]):
        name = self.name
        ...
        self.track.setDoneEvent(self.track.getName())
        self.acceptOnce(
            self.track.getName(), self.finishTeleportOut, [
                callback, extraArgs])
    ...
    def enterTeleportedOut(self, animMultiplier=1, ts=0,
                           callback=None, extraArgs=[]):
        self.setActiveShadow(0)
    ...
    def getDiedInterval(self, autoFinishTrack=1):
        ...
        ival = Sequence(
            Func(
                Emote.globalEmote.disableBody, self), Func(
                self.sadEyes), Func(
                self.blinkEyes), Track(
                    (0, ActorInterval(
                        self, 'lose')), (2, SoundInterval(
                            sound, node=self)), (5.333, self.scaleInterval(
                                1.5, VBase3(
                                    0.01, 0.01, 0.01), blendType='easeInOut'))), Func(
                                        self.detachNode), Func(
                                            self.setScale, 1, 1, 1), Func(
                                                self.normalEyes), Func(
                                                    self.blinkEyes), Func(
                                                        Emote.globalEmote.releaseBody, self), name=trackName, autoFinish=autoFinishTrack)
        return ival
    ...
    def getTeleportInTrack(self):
        ...
        toonTrack = Sequence(
            Wait(0.3), Func(
                self.getGeomNode().show), Func(
                self.nametag3d.show), ActorInterval(
                self, 'jump', startTime=0.45))
        if hasattr(self, 'uniqueName'):
    ...
    def enterTeleportIn(self, animMultiplier=1, ts=0,
                        callback=None, extraArgs=[]):
        if self.ghostMode or self.isDisguised:
            if callback:
                callback(*extraArgs)
    ...
    def enterSitStart(self, animMultiplier=1, ts=0,
                      callback=None, extraArgs=[]):
    ...
    def doEmote(self, emoteIndex, animMultiplier=1,
                ts=0, callback=None, extraArgs=[]):
        if not self.isLocal():
    ...
    def enterFallDown(self, animMultiplier=1, ts=0,
                      callback=None, extraArgs=[]):
    ...
    def stunToon(self, ts=0, callback=None, knockdown=0):
        if not self.isStunned:
            ...
            track = Sequence(
                Func(
                    setStunned, 1), down, up, down, up, down, up, down, clear, Func(
                    self.restoreDefaultColorScale), Func(
                    setStunned, 0))
            if knockdown:
        ...
        if self.cheesyEffect != effect:
            ...
            if oldEffect == ToontownGlobals.CENormal:
                ...
                self.effectTrack = Sequence(
                    self.__undoCheesyEffect(
                        oldEffect,
                        lerpTime / 2.0),
                    self.__doCheesyEffect(
                        effect,
                        lerpTime / 2.0))
    ...
    def reapplyCheesyEffect(self, lerpTime=0):
        ...
        self.effectTrack = Sequence(
            self.__undoCheesyEffect(
                effect, 0), self.__doCheesyEffect(
                effect, lerpTime))
        self.effectTrack.start()
        return
    ...
    def __doPumpkinHeadSwitch(self, lerpTime, toPumpkin):
        ...
        if toPumpkin:
            ...
            def hideParts():
                ...
                for head in self.headParts:
                    for p in head.getChildren():
                        if hasattr(self, 'pumpkins') and not self.pumpkins.hasPath(
                                p):
                            p.hide()
            ...
            def showHiddenParts():
                ...
                for head in self.headParts:
                    for p in head.getChildren():
                        if not self.pumpkins.hasPath(
                                p) and p.getTag('pumpkin') == 'enabled':
                            p.show()
        ...
        if toSnowMan:
            ...
            def showHiddenParts():
                ...
                for hi in range(self.headParts.getNumPaths()):
                    ...
                    for pi in range(parts.getNumPaths()):
                        p = parts[pi]
                        if not self.snowMen.hasPath(
                                p) and p.getTag('snowman') == 'enabled':
                            p.show()
    ...
    def __doGreenToon(self, lerpTime, toGreen):
        ...
        if toGreen:
            ...
            greenTrack.append(
                self.__colorToonEars(
                    skinGreen,
                    muzzleGreen,
                    lerpTime))
            greenTrack.append(
                self.__colorScaleToonMuzzle(
                    muzzleGreen, lerpTime))
    ...
    def __doPartsColorScale(self, scale, lerpTime):
        ...
        pieces = self.getPieces(
            ('torso', ('arms', 'neck')), ('legs', ('legs', 'feet')), ('head', '+GeomNode'))
        track = Sequence()
        track.append(Func(node.setTransparency, 1))
        for piece in pieces:
            if piece.getName()[
    ...
    def __doCheesyEffect(self, effect, lerpTime):
        if effect == ToontownGlobals.CEBigHead:
            ...
            if localAvatar.seeGhosts:
                alpha = 0.2
            return Sequence(self.__doToonGhostColorScale(
                VBase4(1, 1, 1, alpha), lerpTime, keepDefault=1), Func(self.nametag3d.hide))
        return Sequence()
    ...
    def __undoCheesyEffect(self, effect, lerpTime):
        ...
        if effect == ToontownGlobals.CEBigLegs:
            ...
            return Sequence(Func(self.nametag3d.show), self.__doToonGhostColorScale(
    ...
    def putOnSuit(self, suitType, setDisplayName=True, rental=False) -> None:
        ...
        if setDisplayName:
            ...
            suitName: str = SuitBattleGlobals.SuitAttributesDict[suitType].name
    ...
    def getPresentPieInterval(self, x, y, z, h, p, r):
        ...
        track = Sequence(
            Func(
                self.setPosHpr, x, y, z, h, p, r), Func(
                pie.reparentTo, self.rightHand), Func(
                pie.setPosHpr, 0, 0, 0, 0, 0, 0), Parallel(
                    pie.scaleInterval(
                        1, self.pieScale, startScale=MovieUtil.PNT3_NEARZERO), ActorInterval(
                            self, 'throw', startFrame=0, endFrame=31), animPie), Func(
                                self.pingpong, 'throw', fromFrame=32, toFrame=47), pingpongPie)
        return track
        ...
        toss = Track(
            (0, Sequence(
                Func(
                    ...
                    pie.setPosHpr, 0, 0, 0, 0, 0, 0), Parallel(
                        ActorInterval(
                            self, 'throw', startFrame=48), animPie), Func(
                                self.loop, 'neutral'))), (16.0 / 24.0, Func(
                                    pie.detachNode)))
        fly = Track(
            (14.0 / 24.0,
             SoundInterval(
    ...
    def getPieSplatInterval(self, x, y, z, pieCode):
        ...
        ival = Parallel(
            Func(
                ...
                splat.setPos, x, y, z), SoundInterval(
                sound, node=splat, volume=vol), Sequence(
                    ActorInterval(
    ...
    def getScratchPetIval(self):
        return Sequence(ActorInterval(self, 'pet-start'),
                        ActorInterval(self, 'pet-loop'), ActorInterval(self, 'pet-end'))

    def getCallPetIval(self):
        return ActorInterval(self, 'callPet')

    def enterGolfPuttLoop(self, animMultiplier=1, ts=0,
                          callback=None, extraArgs=[]):
        self.loop('loop-putt')
    ...
    def enterGolfRotateLeft(self, animMultiplier=1, ts=0,
                            callback=None, extraArgs=[]):
    ...
    def enterGolfRotateRight(self, animMultiplier=1,
                             ts=0, callback=None, extraArgs=[]):
    ...
    def enterGolfPuttSwing(self, animMultiplier=1, ts=0,
                           callback=None, extraArgs=[]):
    ...
    def enterGolfGoodPutt(self, animMultiplier=1, ts=0,
                          callback=None, extraArgs=[]):
    ...
    def enterGolfBadPutt(self, animMultiplier=1, ts=0,
                         callback=None, extraArgs=[]):
    ...
    def enterCogThiefRunning(self, animMultiplier=1,
                             ts=0, callback=None, extraArgs=[]):
        self.playingAnim = None
        self.playingRate = None
        self.standWalkRunReverse = (('neutral', 1.0),
    ...
    def enterScientistJealous(self, animMultiplier=1,
                              ts=0, callback=None, extraArgs=[]):
