...
class ToonHead(Actor.Actor):
    ...
    EyesOpen = loader.loadTexture(
        '../../user/default/resources/default/phase_3/maps/eyes.jpg',
        '../../user/default/resources/default/phase_3/maps/eyes_a.rgb')
    EyesOpen.setMinfilter(Texture.FTLinear)
    EyesOpen.setMagfilter(Texture.FTLinear)
    EyesClosed = loader.loadTexture(
        '../../user/default/resources/default/phase_3/maps/eyesClosed.jpg',
        '../../user/default/resources/default/phase_3/maps/eyesClosed_a.rgb')
    EyesClosed.setMinfilter(Texture.FTLinear)
    EyesClosed.setMagfilter(Texture.FTLinear)
    EyesSadOpen = loader.loadTexture(
        '../../user/default/resources/default/phase_3/maps/eyesSad.jpg',
        '../../user/default/resources/default/phase_3/maps/eyesSad_a.rgb')
    EyesSadOpen.setMinfilter(Texture.FTLinear)
    EyesSadOpen.setMagfilter(Texture.FTLinear)
    EyesSadClosed = loader.loadTexture(
        '../../user/default/resources/default/phase_3/maps/eyesSadClosed.jpg',
        '../../user/default/resources/default/phase_3/maps/eyesSadClosed_a.rgb')
    EyesSadClosed.setMinfilter(Texture.FTLinear)
    EyesSadClosed.setMagfilter(Texture.FTLinear)
    EyesAngryOpen = loader.loadTexture(
        '../../user/default/resources/default/phase_3/maps/eyesAngry.jpg',
        '../../user/default/resources/default/phase_3/maps/eyesAngry_a.rgb')
    EyesAngryOpen.setMinfilter(Texture.FTLinear)
    EyesAngryOpen.setMagfilter(Texture.FTLinear)
    EyesAngryClosed = loader.loadTexture(
        '../../user/default/resources/default/phase_3/maps/eyesAngryClosed.jpg',
        '../../user/default/resources/default/phase_3/maps/eyesAngryClosed_a.rgb')
    ...
    EyesSurprised = loader.loadTexture(
        '../../user/default/resources/default/phase_3/maps/eyesSurprised.jpg',
        '../../user/default/resources/default/phase_3/maps/eyesSurprised_a.rgb')
    EyesSurprised.setMinfilter(Texture.FTLinear)
    EyesSurprised.setMagfilter(Texture.FTLinear)
    Muzzle = loader.loadTexture('../../user/default/resources/default/phase_3/maps/muzzleShrtGeneric.jpg')
    Muzzle.setMinfilter(Texture.FTLinear)
    Muzzle.setMagfilter(Texture.FTLinear)
    MuzzleSurprised = loader.loadTexture(
        '../../user/default/resources/default/phase_3/maps/muzzleShortSurprised.jpg')
    ...
    LeftAD = Point3(LeftA[0] - LeftA[2] * (LeftD[0] -
                    LeftA[0]) / (LeftD[2] - LeftA[2]), 0.0, 0.0)
    LeftBC = Point3(LeftB[0] - LeftB[2] * (LeftC[0] -
                    LeftB[0]) / (LeftC[2] - LeftB[2]), 0.0, 0.0)
    RightAD = Point3(RightA[0] - RightA[2] * (RightD[0] -
                     RightA[0]) / (RightD[2] - RightA[2]), 0.0, 0.0)
    RightBC = Point3(RightB[0] - RightB[2] * (RightC[0] -
                     RightB[0]) / (RightC[2] - RightB[2]), 0.0, 0.0)
    ...
    def __init__(self):
        try:
            ...
            self.eyelids = ClassicFSM('eyelids', [State('off', self.enterEyelidsOff, self.exitEyelidsOff, ['open', 'closed', 'surprised']),
                                                  State(
                'open', self.enterEyelidsOpen, self.exitEyelidsOpen, [
    ...
    def generateToonHead(self, copy, style, lods, forGui=0):
        ...
        if len(lods) == 1:
            ...
            for lod in lods:
                self.loadModel('../../user/default/resources/default/phase_3' + filePrefix + lod, 'head', lod, copy)
                if not forGui:
    ...
    def loadPumpkin(self, headStyle, lod, copy):
        if hasattr(base, 'launcher') and (
            ...
            ppath = '../../user/default/resources/default/phase_4/models/estate/pumpkin_'
            if headStyle == 'l':
                if copy:
            ...
            ToonHead.notify.debug(
                '../../user/default/resources/default/phase_4 not complete yet. Postponing pumpkin head load.')
    ...
    def loadSnowMan(self, headStyle, lod, copy):
        if hasattr(base, 'launcher') and (
            ...
            snowManPath = '../../user/default/resources/default/phase_4/models/props/tt_m_efx_snowmanHead_'
            if headStyle == 'l':
            ...
            ToonHead.notify.debug('../../user/default/resources/default/phase_4 not loaded yet.')
        ...
        if not self.__eyes.isEmpty():
            ...
            if lp.isEmpty() or rp.isEmpty():
                ...
                lmat = Mat4(0.802174, 0.59709, 0, 0, -
                            0.586191, 0.787531, 0.190197, 0, 0.113565, -
                            0.152571, 0.981746, 0, -
                            0.233634, 0.418062, 0.0196875, 1)
                leye.setMat(lmat)
                rmat = Mat4(0.786788, -
                            0.617224, 0, 0, 0.602836, 0.768447, 0.214658, 0, -
                            0.132492, -
    ...
    def __setPupilDirection(self, x, y):
        ...
        left0 = Point3(0.0, 0.0, left1[2] - left1[0] *
                       (left2[2] - left1[2]) / (left2[0] - left1[0]))
        right0 = Point3(0.0, 0.0, right1[2] -
                        right1[0] *
                        (right2[2] -
                         right1[2]) /
                        (right2[0] -
    ...
    def __lookHeadAt(self, node, point, frac=1.0, lod=None):
        ...
        if decomposeMatrix(rot, scale, hpr, CSDefault):
            hpr = VBase3(min(max(hpr[0], -60), 60),
                         min(max(hpr[1], -20), 30), 0)
            if frac != 1:
                currentHpr = head.getHpr()
                reachedTarget = abs(
                    hpr[0] -
                    currentHpr[0]) < 1.0 and abs(
                    hpr[1] -
                    currentHpr[1]) < 1.0
    ...
    def setupEyelashes(self, style):
        if style.getGender() == 'm':
            ...
            if self.__eyelashClosed:
                self.__eyelashClosed.removeNode()
            animal = style.head[0]
            model = loader.loadModel('../../user/default/resources/default/phase_3' + EyelashDict[animal])
            if self.hasLOD():
    ...
    def setupMuzzles(self, style):
        ...
        if self.hasLOD():
                ...
                if animal != 'dog':
                    ...
                    if lodName == '1000' or lodName == '500':
                        ...
                        muzzles = loader.loadModel(
                            '../../user/default/resources/default/phase_3' + filePrefix + lodName)
                        if ConfigVariableBool('want-new-anims', 1).value:
                            if not self.find(
            ...
            if style.getAnimal() != 'dog':
                ...
                filePrefix = DogMuzzleDict[style.head]
                muzzles = loader.loadModel('../../user/default/resources/default/phase_3' + filePrefix + '1000')
                if ConfigVariableBool('want-new-anims', 1).value:
