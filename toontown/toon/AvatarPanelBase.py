...
class AvatarPanelBase(AvatarPanel.AvatarPanel):
    ...
    def getIgnoreButtonInfo(self):
        if base.cr.avatarFriendsManager.checkIgnored(self.avId):
            ...
            return (TTLocalizer.AvatarPanelIgnore,
                    self.handleIgnore, IGNORE_SCALE)
    ...
    def handleStopIgnoring(self):
        self.dialog = TTDialog.TTGlobalDialog(
            style=TTDialog.TwoChoice,
            text=TTLocalizer.IgnorePanelRemoveIgnore %
            self.avName,
            text_wordwrap=18.5,
            text_scale=0.06,
        ...
        DirectLabel(
            ...
            pos=(
                0,
                TTLocalizer.APBdirectLabelPosY,
                0.15),
            text=TTLocalizer.IgnorePanelTitle,
            textMayChange=0,
            text_scale=0.08)
    ...
    def handleIgnoreConfirm(self, value):
        ...
        self.dialog = TTDialog.TTGlobalDialog(
            ...
            text=TTLocalizer.IgnorePanelIgnore %
            self.avName,
            text_wordwrap=18.5,
            text_scale=0.06,
            topPad=0.1,
    ...
    def handleStopIgnoringConfirm(self, value):
        ...
        self.dialog = TTDialog.TTGlobalDialog(
            style=TTDialog.Acknowledge,
            text=TTLocalizer.IgnorePanelEndIgnore %
            self.avName,
            text_wordwrap=18.5,
            text_scale=0.06,
            topPad=0.1,
    ...
    def confirmReport(self):
        if base.cr.isFriend(
                self.avId) or base.cr.playerFriendsManager.isPlayerFriend(self.avId):
    ...
    def alreadyReported(self):
        self.dialog = TTDialog.TTGlobalDialog(
            style=TTDialog.Acknowledge,
            text=TTLocalizer.ReportPanelAlreadyReported %
            self.avName,
            text_wordwrap=18.5,
            text_scale=0.06,
            topPad=0.1,
    ...
    def requestStopped(self):
        if not base.cr.playGame.getPlace().fsm.getCurrentState().getName() == 'stickerBook':
            if base.cr.playGame.getPlace().fsm.hasStateNamed('stopped'):
                ...
                self.notify.warning(
                    'skipping request to stopped in %s' %
