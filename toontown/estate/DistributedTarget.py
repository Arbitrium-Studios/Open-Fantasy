...
class DistributedTarget(DistributedObject.DistributedObject):
    ...
    def load(self):
        ...
        self.timer.hide()
        self.geom = loader.loadModel('../../user/default/resources/default/phase_5.5/models/estate/target')
        self.geom.reparentTo(base.cr.playGame.hood.loader.geom)
        self.geom.setPos(0, 0, 40)
        self.geom.setScale(3)
        self.geom.stash()
        self.hitSound = base.loader.loadSfx('../../user/default/resources/default/phase_4/audio/sfx/MG_Tag_A.ogg')
    ...
    def handleHitTarget(self, avId=None, vel=None):
        ...
        if vel:
            ...
            self.targetBounceTrack = Sequence(
                LerpPosInterval(
                    self.geom, duration=0.1, pos=newPos, blendType='easeOut'), LerpPosInterval(
                    self.geom, duration=0.25, pos=springPos, blendType='easeOut'), LerpPosInterval(
                    self.geom, duration=0.2, pos=pos, blendType='easeOut'))
    ...
    def __showOnscreenMessage(self, titleText, scoreText):
        ...
        if not self.onscreenMessage:
            ...
            titles = DirectLabel(parent=self.onscreenMessage, relief=None, text=titleText, text_fg=VBase4(
                0, 0, 0, 1), text_align=TextNode.ALeft, text_scale=0.7, pos=(-5.75, 0, 0.5))
            scores = DirectLabel(
                parent=self.onscreenMessage, relief=None, text=scoreText, text_fg=VBase4(
                    1, 0, 0, 1), text_align=TextNode.ARight, text_scale=0.7, pos=(
                    5.75, 0, 0.5))
