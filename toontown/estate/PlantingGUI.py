...
def loadJellyBean(parent, beanIndex):
    gui = loader.loadModel('../../user/default/resources/default/phase_5.5/models/estate/jellyBean')
...
class PlantingGUI(DirectFrame):
    ...
    def __init__(self, doneEvent, specialBoxActive=False):
        ...
        self.okButton = DirectButton(
            parent=self, relief=None, image=okImageList, pos=(
                ...
                0, -0.1), command=self.__doPlant)
        self.resetButton = DirectButton(
            parent=self, relief=None, image=resetImageList, pos=(
                0.0, 0, -0.35), text=TTLocalizer.PlantingGuiReset, text_scale=0.06, text_pos=(
                0, -0.1), command=self.__reset)
        buttons.removeNode()
    ...
    def createSpecialBox(self, xPos, zPos):
        ...
        self.specialButton = GenericBoxScrollList(
            self.specialButtonFrame, items, incButton_pos=(
                0, 0, -0.135), incButton_scale=(
                0.75, 1.0, -1.0), decButton_pos=(
                0, 0, 0.135), decButton_scale=(
    ...
    def createScrollingBeanBox(self, index, xPos, zPos, active):
        ...
        boxFrame = DirectFrame(
            parent=self, pos=(
                xPos, 0, zPos), geom=DGG.getDefaultDialogGeom(), geom_scale=(
                0.1, 1.0, 0.1), geom_color=geomColor, relief=None)
        items = []
        if active:
        ...
        box = GenericBoxScrollList(
            boxFrame, items, incButton_pos=(
                0, 0, -0.07), incButton_scale=(
                0.4, 1.0, -1.0), decButton_pos=(
                0, 0, 0.065), decButton_scale=(
                    0.4, 1.0, 1.0))
