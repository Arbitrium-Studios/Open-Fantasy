...
class FishSellGUI(DirectFrame):
    ...
    def __init__(self, doneEvent):
        ...
        self.cancelButton = DirectButton(
            parent=self, relief=None, image=cancelImageList, pos=(
                0.3, 0, -0.58), text=TTLocalizer.FishGuiCancel, text_scale=TTLocalizer.FSGUIcancelButton, text_pos=(
                0, -0.1), command=self.__cancel)
        self.okButton = DirectButton(
            parent=self, relief=None, image=okImageList, pos=(
                0.6, 0, -0.58), text=TTLocalizer.FishGuiOk, text_scale=TTLocalizer.FSGUIokButton, text_pos=(
                0, -0.1), command=self.__sellFish)
        self.rewardDoubledJellybeanLabel = DirectLabel(
            text='', text_fg=(
                1.0, 0.125, 0.125, 1.0), relief=None, pos=(
                0.45, 0, -0.48), scale=0.07)
        buttons.removeNode()
    ...
    def __updateFishValue(self):
        doubledJellybean = ''
        if base.cr.newsManager.isHolidayRunning(ToontownGlobals.JELLYBEAN_FISHING_HOLIDAY) or base.cr.newsManager.isHolidayRunning(
