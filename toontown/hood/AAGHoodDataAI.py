...
class AAGHoodDataAI(HoodDataAI.HoodDataAI):
    notify = DirectNotifyGlobal.directNotify.newCategory('AAGHoodDataAI')
    ...
    def findAndCreateGolfKarts(
        ...
        golfKarts = []
        golfKartGroups = []
        if isinstance(dnaGroup, DNAGroup) and dnaGroup.getName().find(
                type) >= 0:
