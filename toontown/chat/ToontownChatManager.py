...
class ToontownChatManager(ChatManager.ChatManager):
    ...
    def __init__(self, cr, localAvatar):
        ...
        self.openScSfx = loader.loadSfx(
            '../../user/default/resources/default/phase_3.5/audio/sfx/GUI_quicktalker.ogg')
        ...
        self.whisperCancelButton = DirectButton(
            parent=self.whisperFrame, image=(
                gui.find('**/CloseBtn_UP'), gui.find('**/CloseBtn_DN'), gui.find('**/CloseBtn_Rllvr')), pos=(
                0.125, 0, -0.1), scale=1.179, relief=None, text=(
                '', OTPLocalizer.ChatManagerCancel, OTPLocalizer.ChatManagerCancel), text_scale=0.05, text_fg=(
                    0, 0, 0, 1), text_pos=(
                        0, -0.09), textMayChange=0, command=self.__whisperCancelPressed)
        gui.removeNode()
    ...
    def enterUnpaidChatWarning(self):
        ...
        if self.unpaidChatWarning is None:
            ...
            self.payButton = DirectButton(
                self.unpaidChatWarning, image=buttonImage, relief=None, text=payButtonText, image_scale=(
                    1.75, 1, 1.15), text_scale=TTLocalizer.TCMpayButton, text_pos=(
                    0, -0.02), textMayChange=0, pos=TTLocalizer.TCMpayButtonPos, command=self.__handleUnpaidChatWarningPay)
            DirectButton(
    ...
    def enterActivateChat(self):
        if self.activateChatGui is None:
            ...
            moreButtonImage = (
                ...
                guiButton.find('**/QuitBtn_DN'),
                guiButton.find('**/QuitBtn_RLVR'))
            nameShopGui = loader.loadModel('../../user/default/resources/default/phase_3/models/gui/nameshop_gui')
            ...
            if base.cr.productName != 'JP':
                DirectButton(
                    self.activateChatGui, image=moreButtonImage, image_scale=(
                        1.25, 1.0, 1.0), relief=None, text=OTPLocalizer.ActivateChatMoreInfo, text_scale=0.06, text_pos=(
                        0, -0.02), textMayChange=0, pos=(
                        0.0, 0.0, -0.7), command=self.__handleActivateChatMoreInfo)
    ...
    def __normalButtonPressed(self):
        ...
        if base.cr.productName in ['DisneyOnline-US', 'ES']:
            ...
            print(
                'ChatManager: productName: %s not recognized' %
                base.cr.productName)
