...
# These are the default values generated with spellbook.json to help get
# them started
...
class ToontownMagicWordManagerAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def requestExecuteMagicWord(
        ...
        if hasattr(self.air, 'tutorialManager') and avId in list(
                self.air.tutorialManager.avId2fsm.keys()):
        ...
        if affectType in (AFFECT_ZONE, AFFECT_SERVER, AFFECT_RANK):
            ...
            for doId in list(self.air.doId2do.keys())[:]:
                ...
                if isinstance(
                        do, DistributedPlayerAI) and do.isPlayerControlled() and do != toon:
        ...
        if len(targetIds) == 0:
            ...
            if (affectRange in (AFFECT_OTHER, AFFECT_BOTH)
                ...
                returnValue = returnValue.format(
                    lastClickedToon.getName(), parsedTargetAccess, parsedInvokerAccess)
