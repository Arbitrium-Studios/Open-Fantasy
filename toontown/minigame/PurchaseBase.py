...
class PurchaseBase(StateData.StateData):
    ...
    def __init__(self, toon, doneEvent):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            'Purchase', [
                State.State(
                    'purchase', self.enterPurchase, self.exitPurchase, ['done']), State.State(
                    'done', self.enterDone, self.exitDone, ['purchase'])], 'done', 'done')
        self.fsm.enterInitialState()

    def load(self, purchaseModels=None):
        if purchaseModels is None:
            purchaseModels = loader.loadModel(
                '../../user/default/resources/default/phase_4/models/gui/purchase_gui')
        self.music = base.loader.loadMusic('../../user/default/resources/default/phase_4/audio/bgm/FF_safezone.ogg')
        ...
        self.pointDisplay = DirectLabel(
            parent=self.frame, relief=None, pos=(
                -1.15, 0.0, 0.16), text=str(
                self.toon.getMoney()), text_scale=0.2, text_fg=(
                0.95, 0.95, 0, 1), text_shadow=(
                    0, 0, 0, 1), text_pos=(
                        0, -0.1, 0), image=self.jarImage, text_font=ToontownGlobals.getSignFont())
