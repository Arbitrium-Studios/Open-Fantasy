...
def npcMatches(toNpcId, npc):
    return toNpcId == npc.getNpcId() or toNpcId == Any or toNpcId == ToonHQ and npc.getHq(
    ) or toNpcId == ToonTailor and npc.getTailor()
...
class Quest:
    ...
    def checkBuildingFloors(self, floors):
        self.check(
            floors >= 1 and floors <= 5,
            'invalid num floors: %s' %
            floors)
    ...
    def checkGagTrack(self, track):
        self.check(track >= ToontownBattleGlobals.MIN_TRACK_INDEX and track <=
                   ToontownBattleGlobals.MAX_TRACK_INDEX, 'invalid gag track: %s' % track)

    def checkGagItem(self, item):
        self.check(item >= ToontownBattleGlobals.MIN_LEVEL_INDEX and item <=
                   ToontownBattleGlobals.MAX_LEVEL_INDEX, 'invalid gag item: %s' % item)
    ...
    def checkTrackChoice(self, option):
        self.check(option >= ToontownBattleGlobals.MIN_TRACK_INDEX and option <=
                   ToontownBattleGlobals.MAX_TRACK_INDEX, 'invalid track option: %s' % option)
...
class CogQuest(LocationBasedQuest):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        ...
        return (questCogType is Any or questCogType is cogDict['type']
                ) and avId in cogDict['activeToons'] and self.isLocationMatch(zoneId)
...
class CogTrackQuest(CogQuest):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        ...
        return questCogTrack == cogDict['track'] and avId in cogDict['activeToons'] and self.isLocationMatch(
            zoneId)
...
class CogLevelQuest(CogQuest):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        ...
        return questCogLevel <= cogDict['level'] and avId in cogDict['activeToons'] and self.isLocationMatch(
            zoneId)
...
class SkelecogQBase:
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        return cogDict['isSkelecog'] and avId in cogDict['activeToons'] and self.isLocationMatch(
            zoneId)
...
class SkelecogTrackQuest(CogTrackQuest, SkelecogQBase):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        return SkelecogQBase.doesCogCount(
            self, avId, cogDict, zoneId, avList) and self.getCogTrack() == cogDict['track']
...
class SkelecogLevelQuest(CogLevelQuest, SkelecogQBase):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        return SkelecogQBase.doesCogCount(
            self, avId, cogDict, zoneId, avList) and self.getCogLevel() <= cogDict['level']
...
class SkeleReviveQBase:
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        return cogDict['hasRevives'] and avId in cogDict['activeToons'] and self.isLocationMatch(
            zoneId)
...
class ForemanQuest(CogQuest):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        return bool(CogQuest.doesCogCount(self, avId, cogDict,
                    zoneId, avList) and cogDict['isForeman'])
...
class SupervisorQuest(CogQuest):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        return bool(CogQuest.doesCogCount(self, avId, cogDict,
                    zoneId, avList) and cogDict['isSupervisor'])
...
class DeliverGagQuest(Quest):
    ...
    def getCompletionStatus(self, av, questDesc, npc=None):
        ...
        track = gag[0]
        level = gag[1]
        questComplete = npc and av.inventory and av.inventory.numItem(
            track, level) >= num
        return getCompleteStatusWithNpc(questComplete, toNpcId, npc)
...
class MinigameNewbieQuest(Quest, NewbieQuest):
    ...
    def getObjectiveStrings(self):
        return [TTLocalizer.QuestsMinigameNewbieQuestString %
...
QuestDict = {
    ...
    121: (TT_TIER,
          OBSOLETE,
          (RecoverItemQuest,
           ToontownGlobals.ToontownCenter,
           1,
           2,
           VeryEasy,
           Any,
    ...
    5001: (BR_TIER,
           ...
           400,
           NA,
           TTLocalizer.TundraWonderlandTrackQuestDict),
    5002: (BR_TIER,
           Cont,
           (TrackChoiceQuest,
            ToontownBattleGlobals.SOUND_TRACK,
...
class CheesyEffectReward(Reward):
    ...
    def getString(self):
        ...
        if hoodId == 0:
            ...
            whileStr = TTLocalizer.CheesyEffectExceptIn % TTLocalizer.ToontownCenter[-1]
        else:
            hoodName = base.cr.hoodMgr.getFullnameFromId(hoodId)
...
RewardDict = {100: (MaxHpReward, 1),
              ...
              300: (TeleportReward, ToontownGlobals.ToontownCenter),
              301: (TeleportReward, ToontownGlobals.ToontownShipyards),
