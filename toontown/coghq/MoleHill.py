...
class MoleHill(NodePath):
    ...
    def loadModel(self):
        self.hill = loader.loadModel('../../user/default/resources/default/phase_12/models/bossbotHQ/mole_hole')
        self.hill.setZ(0.0)
        ...
        self.mole.setZ(-2.5)
        self.moleHead = loader.loadModel('../../user/default/resources/default/phase_12/models/bossbotHQ/mole_norm')
    ...
    def setHillType(self, type):
        if self.isUp and (self.hillType == MoleFieldBase.HILL_MOLE and type ==
                          MoleFieldBase.HILL_BOMB or self.hillType == MoleFieldBase.HILL_BOMB and type == MoleFieldBase.HILL_MOLE):
            return
        self.hillType = type
        self.moleHead.removeNode()
        if type == MoleFieldBase.HILL_MOLE:
            self.moleHead = loader.loadModel(
                '../../user/default/resources/default/phase_12/models/bossbotHQ/mole_norm')
        ...
        if type == MoleFieldBase.HILL_BOMB or type == MoleFieldBase.HILL_COGWHACKED:
            self.moleHead = loader.loadModel(
                '../../user/default/resources/default/phase_12/models/bossbotHQ/mole_cog')
            self.moleColNodePath.setScale(1.0)
            self.mole.setBillboardAxis(localAvatar, 0)
            if type == MoleFieldBase.HILL_COGWHACKED:
                ...
                self.popIval = Sequence(
                    Parallel(
                        Sequence(
                            LerpPosInterval(
                                self.moleHead, 0.05, Point3(
                                    0.28, 0.0, 0.0)), LerpPosInterval(
                                self.moleHead, 0.05, Point3(
                                    0.0, -0.23, 0.0)), LerpPosInterval(
                                self.moleHead, 0.05, Point3(
                                    0.0, 0.0, 0.28)), LerpPosInterval(
                                        self.moleHead, 0.05, Point3(
                                            -0.35, 0.0, 0.0)), LerpPosInterval(
                                                self.moleHead, 0.05, Point3(
                                                    0.0, 0.28, 0.0)), LerpPosInterval(
                                                        self.moleHead, 0.05, Point3(
                                                            0.31, 0.0, 0.0)), LerpPosInterval(
                                                                self.moleHead, 0.05, Point3(
                                                                    0.0, -0.32, 0.0)), LerpPosInterval(
                                                                        self.moleHead, 0.05, Point3(
                                                                            0.0, 0.0, 0.48)), LerpPosInterval(
                                                                                self.moleHead, 0.05, Point3(
                                                                                    -0.28, 0.0, 0.0)), LerpPosInterval(
                                                                                        self.moleHead, 0.05, Point3(
                                                                                            0.0, 0.29, 0.0)))), LerpPosInterval(
        ...
        if type == MoleFieldBase.HILL_WHACKED:
            self.moleHead = loader.loadModel(
                '../../user/default/resources/default/phase_12/models/bossbotHQ/mole_hit')
            ...
            self.popIval = Sequence(
                Parallel(
                    Sequence(
                        LerpPosInterval(
                            self.moleHead, 0.05, Point3(
                                0.18, 0.0, 0.0)), LerpPosInterval(
                            self.moleHead, 0.05, Point3(
                                0.0, -0.13, 0.0)), LerpPosInterval(
                            self.moleHead, 0.05, Point3(
                                0.0, 0.0, 0.18)), LerpPosInterval(
                            self.moleHead, 0.05, Point3(
                                -0.15, 0.0, 0.0)), LerpPosInterval(
                            self.moleHead, 0.05, Point3(
                                0.0, 0.18, 0.0)), LerpPosInterval(
                            self.moleHead, 0.05, Point3(
    ...
    def doMolePop(self, startTime, timeToMoveUp,
                  timeToStayUp, timeToMoveDown, moleType):
