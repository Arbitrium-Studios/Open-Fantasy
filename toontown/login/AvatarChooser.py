...
class AvatarChooser(StateData.StateData):
    ...
    def load(self, isPaid):
        ...
        # self.discordButton.hide()
        self.creditsButton = DirectButton(image=(quitHover, quitHover, quitHover), relief=None, text='Credits', text_font=ToontownGlobals.getSignFont(), text_fg=(0.977, 0.816, 0.133, 1), text_scale=(0.075), image_scale=1, image1_scale=1.05, image2_scale=1.05, scale=1.05, text_pos=(0, -0.025), pos=(1.05, 0, -0.9195), command=self.openCredits)
        # self.creditsButton.reparentTo(base.a2dBottomRight)
        self.creditsButton.hide()
        gui.removeNode()
        gui2.removeNode()
        newGui.removeNode()
