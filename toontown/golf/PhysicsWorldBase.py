...
class PhysicsWorldBase:
        ...
        for key in self.commonObjectDict:
            ...
            if entry[1] in [2, 4]:
                ...
                for index in range(len(timeData)):
                    if index == len(
                            timeData) - 1 and timeData[index] < time or timeData[index] < time and timeData[index + 1] > time:
    ...
    def createCommonObject(self, type, commonId, pos, hpr,
                           sizeX=0, sizeY=0, moveDistance=0):
        if commonId is None:
    ...
    def createCross(self, world, space, density, lx, ly, lz,
                    colOnlyBall=0, attachedGeo=None, aHPR=None, aPos=None):
        body = OdeBody(self.world)
        M = OdeMass()
    ...
    def createCross2(self, world, space, density, lx, ly, lz, latSlide,
                     colOnlyBall=0, attachedGeo=None, aHPR=None, aPos=None):
        body = OdeBody(self.world)
        M = OdeMass()
        M.setBox(density, lx, ly, lz)
    ...
    def createPinWheel(self, world, space, density, lx, ly, lz, numBoxes, disV,
                       disH, colOnlyBall=0, attachedGeo=None, aHPR=None, aPos=None, offRot=0):
        body = OdeBody(self.world)
        M = OdeMass()
        M.setBox(density, lx, ly, lz)
        body.setMass(M)
