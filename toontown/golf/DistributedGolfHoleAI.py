...
class DistributedGolfHoleAI(
    ...
    defaultTransitions = {
        'Off': [
            'Cleanup', 'WaitTee'], 'WaitTee': [
            'WaitSwing', 'Cleanup', 'WaitTee', 'WaitPlayback'], 'WaitSwing': [
                'WaitPlayback', 'Cleanup', 'WaitSwing', 'WaitTee'], 'WaitPlayback': [
                    'WaitSwing', 'Cleanup', 'WaitTee', 'WaitPlayback'], 'Cleanup': ['Off']}
    id = 0
    ...
    def postSwingState(self, cycleTime, power, x, y, z, dirX,
                       dirY, curAimTime, commonObjectData):
    ...
    def ballMovie2AI(self, cycleTime, avId, movie, spinMovie, ballInFrame,
                     ballTouchedHoleFrame, ballFirstTouchedHoleFrame, commonObjectData):
        ...
        if sentFromId == self.trustedPlayerId:
            ...
            if self.trustedPlayerId is None:
                return
            else:
                self.doAction()
        self.trustedPlayerId = None
    ...
    def postResult(self, cycleTime, avId, recording, aVRecording,
                   ballInHoleFrame, ballTouchedHoleFrame, ballFirstTouchedHoleFrame):
        pass
    ...
    def isCurBallInHole(self, golferId):
        ...
        if golferId not in self.ballPos:
            self.notify.warning(
                'golferId=%s not in self.ballPos=%s' %
                (golferId, self.ballPos))
            simbase.air.writeServerEvent(
                'suspicious', senderId, 'isCurBallInHole golferId=%s not in self.ballPos=%s' %
                (golferId, self.ballPos))
            return False
    ...
    def setBox(self, pos0, pos1, pos2, quat0, quat1, quat2,
               quat3, anV0, anV1, anV2, lnV0, lnV1, lnV2):
