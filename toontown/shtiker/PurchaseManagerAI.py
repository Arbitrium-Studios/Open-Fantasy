...
class PurchaseManagerAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(self, air, playerArray, mpArray, previousMinigameId, trolleyZone,
                 newbieIdList=[], votesArray=None, metagameRound=-1, desiredNextGame=None):
        ...
        for avId in self.getInvolvedPlayerIds():
            if avId > 3 and avId in self.air.doId2do:
                ...
                if self.metagameRound == TravelGameGlobals.FinalMetagameRoundIndex:
                    ...
                    if self.air.holidayManager.isHolidayRunning(ToontownGlobals.JELLYBEAN_TROLLEY_HOLIDAY) or self.air.holidayManager.isHolidayRunning(
        ...
        if self.receivingButtons:
            if avId in self.air.doId2do:
                ...
                if avIndex is None:
                    ...
                    self.notify.warning(
                        'Avatar ' + str(avId) + ' requested Exit, but is not on the list!')
                else:
            ...
            self.air.writeServerEvent(
                'suspicious', avId, 'PurchaseManager.requestExit not receiving requests now')
            self.notify.warning(
                'Avatar ' +
                str(avId) +
    ...
    def setInventory(self, blob, newMoney, done):
        ...
        if self.receivingInventory:
            ...
            self.air.writeServerEvent(
                'suspicious', avId, 'PurchaseManager.setInventory not receiving inventory')
            self.notify.warning(
