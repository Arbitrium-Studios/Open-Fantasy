...
class DistributedCashbotBossAI(
    ...
    def makeTreasure(self, goon):
        ...
        if goon.strength <= 10:
            style = ToontownGlobals.ToontownCenter
            healAmount = 3
        else:
            if goon.strength <= 15:
                style = random.choice([ToontownGlobals.ToontownShipyards,
                                       ToontownGlobals.FloweringGrove,
                                       ToontownGlobals.TheLandOfMusic])
                healAmount = 10
            else:
                style = random.choice(
                    [ToontownGlobals.TundraWonderland, ToontownGlobals.TwilightDreamland])
                healAmount = 12
