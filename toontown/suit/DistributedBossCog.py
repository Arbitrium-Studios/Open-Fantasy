...
class DistributedBossCog(DistributedAvatar.DistributedAvatar, BossCog.BossCog):
    ...
    def announceGenerate(self):
        ...
        roof = CollisionPolygon(Point3(-4.4, 7.1, 5.5), Point3(-4.4, -7.1, 5.5),
                                Point3(4.4, -7.1, 5.5), Point3(4.4, 7.1, 5.5))
        side1 = CollisionPolygon(Point3(-4.4, -7.1, 5.5), Point3(-4.4,
                                 7.1, 5.5), Point3(-4.4, 7.1, 0), Point3(-4.4, -7.1, 0))
        side2 = CollisionPolygon(Point3(4.4, 7.1, 5.5), Point3(
            4.4, -7.1, 5.5), Point3(4.4, -7.1, 0), Point3(4.4, 7.1, 0))
        front1 = CollisionPolygon(Point3(
            4.4, -7.1, 5.5), Point3(-4.4, -7.1, 5.5), Point3(-4.4, -7.1, 5.2), Point3(4.4, -7.1, 5.2))
        back1 = CollisionPolygon(Point3(-4.4, 7.1, 5.5), Point3(4.4,
                                 7.1, 5.5), Point3(4.4, 7.1, 5.2), Point3(-4.4, 7.1, 5.2))
    ...
    def doZapToon(self, toon, pos=None, hpr=None, ts=0, fling=1, shake=1):
        ...
        if shake and toon == localAvatar:
            toonTrack.append(Sequence(Func(camera.setZ, camera, 1), Wait(0.15), Func(
                camera.setZ, camera, -2), Wait(0.15), Func(camera.setZ, camera, 1)))
        if fling:
            toonTrack += [ActorInterval(toon,
                                        'slip-backward'),
    ...
    def getGearFrisbee(self):
        return loader.loadModel('../../user/default/resources/default/phase_9/models/char/gearProp')
    ...
    def announceAreaAttack(self):
        if not getattr(
                localAvatar.controlManager.currentControls, 'isAirborne', 0):
    ...
    def loadEnvironment(self):
        ...
        self.battleOneMusic = base.loader.loadMusic(
            '../../user/default/resources/default/phase_3.5/audio/bgm/encntr_general_bg.ogg')
        self.battleThreeMusic = base.loader.loadMusic(
            '../../user/default/resources/default/phase_7/audio/bgm/encntr_suit_winning_indoor.ogg')
        self.epilogueMusic = base.loader.loadMusic(
            '../../user/default/resources/default/phase_9/audio/bgm/encntr_hall_of_fame.ogg')
    ...
    def enterWaitForToons(self):
        ...
        self.transitions.IrisModelName = '../../user/default/resources/default/phase_3/models/misc/iris'
        self.transitions.FadeModelName = '../../user/default/resources/default/phase_3/models/misc/fade'
    ...
    def enterIntroduction(self):
        ...
        seq = Sequence(self.makeIntroductionMovie(delayDeletes),
                       Func(self.__beginBattleOne), name=intervalName)
    ...
    def wearCogSuits(self, toons, battleNode, camLoc,
                     arrayOfObjs=False, waiter=False):
        seq = Sequence()
        if not toons:
            return seq
        self.notify.debug('battleNode=%s camLoc=%s' % (battleNode, camLoc))
