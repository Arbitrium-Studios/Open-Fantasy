...
def makeFanfareWithMessage(delay, toon, showToonName,
                           message, messagePos, messageScale, wordwrap=100):
...
def makeFanfareWithImage(delay, toon, showToonName,
                         image, imagePos, imageScale, wordwrap=100):
    panel = makePanel(toon, showToonName)
...
def makeFanfareWithMessageImage(delay, toon, showToonName, message,
                                messagePos, messageScale, image, imagePos, imageScale, wordwrap=100):
    panel = makePanel(toon, showToonName)
    makeMessageBox(panel, message, messagePos, messageScale, wordwrap)
    makeImageBox(panel.itemFrame, image, imagePos, imageScale)
    return doFanfare(delay, toon, panel)
...
def doFanfare(delay, toon, panel):
                          ...
                          Parallel(LerpHprInterval(partyBallLeft,
                                   ...
                                   LerpHprInterval(partyBallRight,
                                                   ...
                                                   hpr=Vec3(-90,
                                                            ...
                                                            0),
                                                   blendType='easeInOut')))
    openBall = Parallel(LerpHprInterval(partyBallLeft, duration=0.2, startHpr=Vec3(90, 0, 0), hpr=Vec3(
        90, 30, 0)), LerpHprInterval(partyBallRight, duration=0.2, startHpr=Vec3(-90, 0, 0), hpr=Vec3(-90, 30, 0)))
    ...
    def getScaleBlendIntervals(
            props, duration, startScale, endScale, blendType):
        tracks = Parallel()
        for prop in props:
            tracks.append(
                LerpScaleInterval(
                ...
                1, 0, 0, 0)), LerpColorScaleInterval(
                    partyBallRight, 0.3, startColorScale=Vec4(
                        1, 1, 0, 1), colorScale=Vec4(
                            1, 1, 0, 0)))
    play = Parallel(SoundInterval(crabHorn,
                                  startTime=1.5,
                                  duration=4.0,
                                  node=toon),
                    Sequence(Wait(0.25),
