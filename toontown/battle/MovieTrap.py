...
def __createPlacedTrapMultiTrack(
        trap, prop, propName, propPos=None, propHpr=None, explode=0, visibleOnlyForThisSuitId=None):
    toon = trap['toon']
    if 'npc' in trap:
        toon = trap['npc']
    level = trap['level']
...
def createThrowingTrack(object, target, duration=1.0,
    ...
    return Sequence(Func(calcOriginAndVelocity), LerpFunctionInterval(
        throwPos, fromData=0.0, toData=1.0, duration=duration, extraArgs=[object, duration, target]))
