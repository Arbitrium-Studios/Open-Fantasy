...
class RewardPanel(DirectFrame):
    ...
    def __init__(self, name):
        ...
        for i in range(ToontownGlobals.MaxQuestCarryLimit):
            label = DirectLabel(
                parent=self.questFrame, relief=None, pos=(
                    -0.85, 0, -0.1 * i), text=TTLocalizer.RewardPanelQuestLabel %
                i, text_scale=0.05, text_align=TextNode.ALeft)
            label.hide()
        ...
        self.congratsLeft = DirectLabel(
            parent=self.newGagFrame, pos=(-0.2, 0, -0.1), text='', text_pos=(0, 0), text_scale=0.06)
        ...
        for i in range(len(SuitDNA.suitDepts)):
            ...
            self.meritIncLabels.append(
                DirectLabel(
                    parent=self.gagExpFrame, relief=None, text='', text_scale=0.05, text_align=TextNode.ALeft, pos=(
                        0.7, 0, -0.09 * i - 0.125), text_pos=(
                        0, -0.02)))
        ...
        for i in range(len(ToontownBattleGlobals.Tracks)):
            ...
            self.trackBars.append(DirectWaitBar(parent=self.gagExpFrame, relief=DGG.SUNKEN, frameSize=(-1,
                                                                                                       ...
                                                                                                       0.15), borderWidth=(0.02, 0.02), scale=0.25, frameColor=(ToontownBattleGlobals.TrackColors[i][0] * 0.7,
                                                                                                                                                                ToontownBattleGlobals.TrackColors[
                                                                                                                                                                    i][1] * 0.7,
                                                                                                                                                                ToontownBattleGlobals.TrackColors[
                                                                                                                                                                    i][2] * 0.7,
        ...
        self._battleGui = loader.loadModel('../../user/default/resources/default/phase_3.5/models/gui/battle_gui')
        self.skipButton = DirectButton(parent=self, relief=None, image=(self._battleGui.find('**/tt_t_gui_gen_skipSectionUp'),
                                                                        ...
                                                                        self._battleGui.find(
                                                                            '**/tt_t_gui_gen_skipSectionRollOver'),
    ...
    def getTrackIntervalList(self, toon, track, origSkill,
                             earnedSkill, hasUber, guestWaste=0):
        if hasUber < 0:
            print(
    ...
    def getPromotionIntervalList(self, toon, dept):
        ...
        intervalList = [
            Func(
                self.promotion, toon, dept), Wait(finalDelay), Func(
                self.cleanupPromotion)]
        return intervalList
    ...
    def getExpTrack(self, toon, origExp, earnedExp, deathList, origQuestsList, itemList, missedItemList,
                    origMeritList, meritList, partList, toonList, uberEntry, helpfulToonsList, noSkip=False):
        track = Sequence(
            Func(
                self.initGagFrame,
                toon,
