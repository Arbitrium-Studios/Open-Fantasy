...
class PropPool:
    ...
    def __init__(self):
        ...
        self.propTypes[propName] = 'actor'
        propName = 'ship'
        self.propStrings[propName] = ('../../user/default/resources/default/phase_5/models/props/ship.bam',)
        self.propTypes[propName] = 'model'
        propName = 'trolley'
        self.propStrings[propName] = (
            '../../user/default/resources/default/phase_4/models/modules/trolley_station_TT',)
        self.propTypes[propName] = 'model'

    def getPath(self, phase, model):
        return '../../user/default/resources/default/phase_%s/models/props/%s' % (phase, model)
    ...
    def makeVariant(self, name):
        if name == 'tart':
            ...
            tex = loader.loadTexture('../../user/default/resources/default/phase_5/maps/dollar_5.jpg')
            tex.setMinfilter(Texture.FTLinearMipmapLinear)
            tex.setMagfilter(Texture.FTLinear)
            self.props[name].setTexture(tex, 1)
        elif name == '10dollar':
            tex = loader.loadTexture('../../user/default/resources/default/phase_5/maps/dollar_10.jpg')
            tex.setMinfilter(Texture.FTLinearMipmapLinear)
            tex.setMagfilter(Texture.FTLinear)
            self.props[name].setTexture(tex, 1)
        elif name == 'dust':
    ...
    def __getPropCopy(self, name):
        if self.propTypes[name] == 'actor':
            if name not in self.props:
                ...
                prop.setName(name)
                prop.setBlend(frameBlend=base.smoothAnimations)
                self.storeProp(name, prop)
                if name in Variants:
                    self.makeVariant(name)
            actorProp = Actor.Actor(other=self.props[name])
            actorProp.setBlend(frameBlend=base.smoothAnimations)
            return actorProp
        else:
            if name not in self.props:
